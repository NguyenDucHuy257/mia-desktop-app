...
test('local account list starts empty, persists in the gateway and supports deletion', async ({ page }) => {
  ...
  await expect(page.locator('.progress-cell')).toContainText('Chưa đồng bộ');
  await expect(page.locator('.sync-state-cell')).toContainText('Chưa đồng bộ');
...
test('detail results retain total rows on page two and empty export is stopped before lifecycle', async ({ page }) => {
  ...
  await page.getByLabel('Chọn nội dung tải xuống').getByText('Tổng quan', { exact: true }).click();
...
test('shows bounded polling failure and lets the user retry', async ({ page }) => {
  await page.addInitScript(() => {
    ...
    const record = { job_id: 'job-retry', connection_id: account.connection_id, intent: {}, idempotency_key: 'desktop-fixed', created_at: 'now', updated_at: 'now', status: 'running' };
    Object.defineProperty(window, 'miaRuntime', { value: {
      accountConnections: { list: async () => [account], create: async () => account, get: async () => account, reconnect: async () => account, revoke: async () => undefined },
      jobs: {
      resume: async () => record,
...
test('opens local overview/detail results and paginates by cursor', async ({ page }) => {
  await page.addInitScript(() => {
    const account = { connection_id: 'conn_results', username: '0101234567', company_name: 'Công ty Kết quả', status: 'connected', token_generation: 1, created_at: 'now', updated_at: 'now', reused: false };
    const record = { job_id: 'job-results', connection_id: account.connection_id, intent: {}, idempotency_key: 'desktop-results', created_at: 'now', updated_at: 'now', status: 'running' };
    const pageFor = (kind: string, cursor?: string | null) => ({
      items: [{ row_id: cursor ? 2 : 1, direction: 'purchase', invoice_key: `${kind}-${cursor ?? 'first'}`, fields: { business_key: `${kind}-${cursor ?? 'first'}`, company: 'Demo' } }],
      columns: ['business_key', 'company'],
      column_labels: { business_key: 'Mã nghiệp vụ', company: 'Công ty' },
      total_count: 2,
    ...
    Object.defineProperty(window, 'miaRuntime', { value: {
      ...
      jobs: {
        resume: async () => record, resumeAll: async () => [record],
        status: async () => ({ job_id: record.job_id, status: 'running', stage: 'detail', overall_percent: 80, current_month: null, updated_at: 'now', error: null }),
        summary: async () => ({}), start: async () => ({ record, accepted: {} }), cancel: async () => ({}), clear: async () => undefined,
      },
      results: {
...
test('date, search, status and pagination controls update the invoice UI', async ({ page }) => {
  await page.goto('/?figma=1');
  await page.getByRole('button', { name: /KHOẢNG THỜI GIAN/ }).click();
...
test('settings persist sequential retry limits and logs are filtered after main-process redaction', async ({ page }) => {
  await page.addInitScript(() => {
    let preferences = { concurrency: 1, retries: 5, exportFolder: 'C:\\MIA' };
    Object.defineProperty(window, 'storedPreferences', { get: () => preferences });
    Object.defineProperty(window, 'miaRuntime', { value: {
  ...
  await expect(page.getByLabel('Chế độ xử lý tuần tự')).toBeDisabled();
  await expect(page.getByLabel('Chế độ xử lý tuần tự')).toHaveValue('1');
  ...
  expect(await page.evaluate(() => (window as typeof window & { storedPreferences: { concurrency: number; retries: number } }).storedPreferences)).toMatchObject({ concurrency: 1, retries: 1 });
  await page.getByRole('button', { name: 'Đóng' }).click();
...
test('PDF tab validates folder and exposes converting/cancel states', async ({ page }) => {
  ...
  const folder = page.getByRole('textbox', { name: 'Thư mục lưu trữ PDF' });
  await folder.fill('');
...
for (const width of [1024, 1280, 1366, 1440, 1500, 1600]) {
  test(`XML HTML layout remains usable at ${width}px`, async ({ page }) => {
    await page.setViewportSize({ width, height: 1024 });
    await page.goto('/?demo=1');
    await page.getByRole('button', { name: 'XML/HTML' }).click();
    const artifactPage = page.locator('.xml-html-page');
    await expect(artifactPage).toBeVisible();
    expect(await artifactPage.evaluate((element) => element.scrollWidth <= element.clientWidth + 1)).toBe(true);
  });
}
