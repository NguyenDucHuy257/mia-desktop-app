...
export function XmlHtmlPage({ accounts, selectedConnectionIds, onSelectAccount, onSelectAccounts, folder, onFolder, lifecycle, selection, onSelectionChange, pdfConcurrency }: {
  ...
  async function startDownload() {
    ...
    if (!selection.directions.length) { setFeedback('Vui lòng chọn ít nhất Mua vào hoặc Bán ra.'); return; }
  ...
  return <section className="artifact-account-page" aria-labelledby="artifact-title">
    ...
    <section className="artifact-toolbar-card" aria-label="Thiết lập tải XML HTML PDF">
      ...
      <div className="artifact-direction-options" aria-label="Loại hóa đơn">{([['purchase', 'Mua vào'], ['sold', 'Bán ra']] as const).map(([direction, label]) => <label key={direction} data-active={selection.directions.includes(direction)}><input type="checkbox" checked={selection.directions.includes(direction)} disabled={lifecycle.active} onChange={() => toggleDirection(direction)} /><span className="selection-box" data-checked={selection.directions.includes(direction)}>{selection.directions.includes(direction) ? '✓' : ''}</span>{label}</label>)}</div>
