...
_DETAIL_EXPORT_COLUMNS: tuple[tuple[str, str], ...] = (
    ("khmshdon", "Mẫu số HD"),
    ("khhdon", "Ký hiệu hóa đơn"),
    ("shdon", "Số hóa đơn"),
...
# Exact A:AK widths from the customer-supplied legacy Detail workbook. These
# values are intentionally static: runtime content must never resize columns.
_DETAIL_EXPORT_WIDTHS: tuple[float, ...] = (
...
def _detail_source_columns(worksheet: Any, header_row: int) -> list[int]:
    from app.exporters.invoice_detail_excel_exporter import InvoiceDetailExcelExporter

    keys = InvoiceDetailExcelExporter._column_keys(worksheet, header_row)
    positions: dict[str, list[int]] = {}
    for column_index, key in enumerate(keys, start=1):
        if key != "stt":
            positions.setdefault(key, []).append(column_index)

    used: dict[str, int] = {}
    source_columns: list[int] = []
    for key, _label in _DETAIL_EXPORT_COLUMNS:
        occurrence = used.get(key, 0)
        candidates = positions.get(key, [])
        if occurrence >= len(candidates):
            raise RuntimeError(f"missing_detail_export_column:{key}")
        source_columns.append(candidates[occurrence])
...
def _normalize_detail_excel_atomically(target: Path, progress=None) -> None:
    ...
    source_workbook = load_workbook(target, data_only=False)
    ...
    for column_index, ((_key, label), width) in enumerate(
        ...
        cell = output_sheet.cell(1, column_index, label)
