...
const JOB_POLICY_NAMESPACE = 'source-api-v1';
...
function createJobLifecycleBroker(getRuntime, now = () => new Date().toISOString(), _protector, createAttemptId = crypto.randomUUID) {
  if (typeof getRuntime !== 'function') throw new TypeError('Invalid offline runtime dependency.');
  const attemptKeys = new Map();
  const startSourceJob = async (intent, key) => getRuntime().invoke('source.jobs.start', {
    intent,
    idempotency_key: key,
  }, {
    // Source ExternalApiService computes coverage/admission before returning.
    // Do not impose the former desktop 15s timeout on the source API contract.
    timeoutMs: 120000,
  });
